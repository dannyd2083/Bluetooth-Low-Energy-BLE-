            ...
            if pair_pub_key[0] == PAIR_PUB_KEY:
                ...
                if Cb_bytes[0] == PAIR_CONF_OPCODE:
                    ...
                    if Nb_bytes[0] == PAIR_RAND_OPCODE:
                        ...
                        Cb_calculated = f4(responder_pub_key_x,x_bytes,Nb,b'\x00') #TODO6
                        ...
                        if Cb_calculated == Cb_received:
                            ...
                            (mackey, ltk) =  f5(dhkey,Na,Nb, b'\x00' + MAC_ADDR, b'\x00' + MAC_ADDR_responder)#TODO7

                            log.info(f'initiator mackey:{mackey.hex()}')
                            log.info(f'initiator ltk:{ltk.hex()}')
                            # Calculate Ea and send it to responder
                            #TODO7
                            Ea = f6(mackey,Na,Nb, p8(0) * 16,p8(IOCap)+p8(OOBDATA)+p8(AuthReq), b'\x00' + MAC_ADDR, b'\x00' + MAC_ADDR_responder) #TODO7
                            ...
                            if Eb_bytes[0] == PAIR_CONF_OPCODE:
                                Eb = Eb_bytes[1:]
                                #TODO7
                                Eb_calculated =  f6(mackey,Nb,Na, p8(0) * 16,iocap_b,b'\x00' +MAC_ADDR_responder,b'\x00' + MAC_ADDR) #DUMMY
